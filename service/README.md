# AssetForge — подробная документация сервиса

> 📌 **Начните с [корневого README](../README.md)** — там быстрый старт, обзор
> архитектуры, скриншоты и общая информация о проекте. Этот файл — углублённая
> техническая документация сервиса (полная структура backend, provenance
> Docker-образов, модели инференса).
...
> Продолжение исследовательской части репозитория (`../docs/`: EDA, `baseline/`,
> бенчмарки): обученные style-LoRA и подобранные пайплайны генерации здесь
> «упакованы» в продуктовый MVP.
...
│   └── tests/                 # unit + integration (42 теста)
...
| Покрытие тестами критичных частей                  | pytest: 42 теста (unit + integration)                             |
